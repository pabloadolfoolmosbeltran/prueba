...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Nuevo Registro de Monitoreo</h1>
    </div>

    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Complete el formulario</h6>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form action="{{ route('registros.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" class="form-control @error('temperatura') is-invalid @enderror"
                           id="temperatura" name="temperatura" value="{{ old('temperatura') }}" required>
                    @error('temperatura')
...
<div class="form-group col-md-6">
    ...
    <input type="number" class="form-control @error('tiempo_operacion') is-invalid @enderror"
           id="tiempo_operacion" name="tiempo_operacion" value="{{ old('tiempo_operacion') }}" min="1" required>
...
{{-- ... resto ... --}}
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control @error('fase') is-invalid @enderror" id="fase" name="fase" required>
                            ...
                            <option value="enfriamiento" {{ old('fase') == 'enfriamiento' ? 'selected' : '' }}>Enfriamiento</option>
                            <option value="fermentacion" {{ old('fase') == 'fermentacion' ? 'selected' : '' }}>Fermentación</option>
                            <option value="reposo" {{ old('fase') == 'reposo' ? 'selected' : '' }}>Reposo</option>
                        </select>
                        @error('fase')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
